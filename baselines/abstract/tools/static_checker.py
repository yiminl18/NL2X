...
def validate_schema_consistency(operators: List[Dict[str, Any]]) -> Tuple[bool, List[str], List[str]]:
    """
        ...
        if 'input' in op and isinstance(op['input'], dict):
            ...
            if isinstance(input_fields, dict):
                for field_name, expected_type in input_fields.items():
                    # Skip checking if this is the first operator (fields come from dataset)
                    if i > 0 and field_name in available_fields:
                    ...
                    elif i > 0 and expected_type != 'Unknown':
                        # Field doesn't exist upstream (unless it's Unknown from dataset)
                        warnings.append(
...
def validate_pipeline(operators: List[Dict[str, Any]], verbose: bool = False) -> Dict[str, Any]:
    """
    ...
    schema_valid, schema_errors, schema_warnings = validate_schema_consistency(operators)
